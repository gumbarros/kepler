...
import 'package:kepler/utils/keplerWalker.dart';
...
class OrbitChartView extends ClusteredChartView<PlanetData> {
  static String getTitle(Cluster data) => (ClusterChart.getMaxForCluster(data) <
              365 ||
          (ClusterChart.getMinForCluster(data) / 365).truncate() == 0)
      ? "${ClusterChart.getMinForCluster(data).toStringAsFixed(2)} - ${ClusterChart.getMaxForCluster(data).toStringAsFixed(2)} days: ${data.instances.length} planets"
      : "${(ClusterChart.getMinForCluster(data) / 365).toStringAsFixed(0)} - ${(ClusterChart.getMaxForCluster(data) / 365).toStringAsFixed(0)} years: ${data.instances.length} planets";
          ...
          leafChartBuilder: (context, cluster, leafDataFutureFunction) =>
              SfCircularChart(
                  onPointTapped: (pointData) async {
                    KeplerWalker.goToPlanetScreenForPlanetNamed(
                      context,
                      cluster.instances[pointData.pointIndex].id,
                    );
                  },
                  onLegendTapped: (pointData) async {
                    KeplerWalker.goToPlanetScreenForPlanetNamed(
